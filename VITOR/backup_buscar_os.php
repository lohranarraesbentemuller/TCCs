...
$.fn.dataTable.ext.search.push(
        ...
        "columnDefs": [
            {
                "render": function ( data, type, row ) { 
                    return " <a  href=\"guia_liberacao.php?id=" + row['id'] + " \"><button class='fa fa-edit btn btn-warning' type='button'></button>"  + row['os']  + "</a>";
                },
                "targets": 1
            },
	...
	"initComplete": function(settings, json) {
		...
		for(i=0;i<p.length;i++){
			var t=p[i].innerHTML;
            a = t.split('<td>')[3].split('</td>')[0];
			a2 = t.split('<td>')[7].split('</td>')[0];
 
			var dia1 = a.split("-")[2];
			var dia2 = a2.split("-")[2];
            //alert(a);
			//alert(a2);
 		   for(i=parseInt(dia1);i<parseInt(dia2);i++)
              saida[i]=saida[i]+1;
