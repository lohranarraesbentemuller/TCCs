   ...
   $(document).ready(function(){
	   $(document).arrive('#upm_escolha',function(){	
		  ...
		  $('#enviar_upm').click(function(){
			  $('#UPM').html('<p>'+$('#upm_escolha option:selected').text()+'</p>');
		  });
	   });
	   ...
	   $('#UPM').dblclick(function(){
		   var s="<select class='form-control' id='upm_escolha' >"
		   <?php $sql="select * from bpms2";
		   $result=mysqli_query($db,$sql);
		   foreach($result as $row)
		   {
			   echo "s+='<option>".$row['nome']."</option>';"."\n";
		   }?>
		   s+="</select><button id='enviar_upm' class='fa fa-check' ></button>"
...
<!--calendario--> 
 <link rel="stylesheet" href="../nepali-date-picker/nepali-date-picker.css" />
<script src="../nepali-date-picker/nepali-date-picker.js"></script>
<h3>Dentro da data inicial e final escolhida, escolha os dias que a OS não estará vigente</h3>
<input name="date1" type="text" class="date-picker form-control" placeholder="Selecione as datas que a OS não estará vigente" data-show_all_dates="true"></button>
...
$(document).ready(function(){
	$(document).arrive('.andp-year-select',function(){	
	
		$('.andp-year-select').val("2021").change();
	});
});
 </script>
<!--calendario--> 
...
$(document).ready(function(){
	 ...
	 $('#enviar_tabela').click(function(){
         ...
         valor+="&n_vigente="+$.trim($('.date-picker').val());
