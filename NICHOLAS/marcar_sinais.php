...
$(document).ready(function(){
    //beatriz
		 valor="";
		 //valor+="x="+$('#circle_'+x).css("left");
		 //valor+="&y="+$('#circle_'+x).css("top");
		 //valor+="&legenda="+$('#sinal_'+x).val();
		 valor+="foto_id=<?php echo $_GET['imagem'];?>";
		 //valor+="&flag=sinal";
	 $.ajax({ 
 type:"POST", 
  url: "carregar_sinais.php", 
 data:valor,
  success: function(data){
	if(data=="")
	{
...
//teste
   const $this = $(this),
      r = 20;//parseInt($('.radius').val().trim(), 10), // Parse as Integer radix 10
      //o = $this.offset(),
      y = data.split('#')[i].split("$")[2];
      x = data.split('#')[i].split("$")[1];
      descricao=data.split('#')[i].split("$")[0];
	  legenda=data.split('#')[i].split("$")[3];
	  if(descricao=="")
	     return;
      sinal.push(descricao);
    ...
    $("<div />", {
      ...
      appendTo: $('#image_preview') // append to #image_preview!
    });
   ...
   // console.log( App.points )
	...
	$('#image_preview').unbind('click',handler);
	contador=contador+1;
...
//teste
  ...
  error: function (jqXHR, exception) {
	...
	$(document).arrive('.identificador',function(){
		//alert("identificador chegou");
		//alert(contador);
		for(i=0;i<contador;i++)
		{
		if ($('#sinal_'+(i)).length == 0) {
			if (typeof sinal[i]==="undefined")
			{
	 	      $('#identificador'+i).append("<table><tr><td><input class='form-control identificador_input' id='sinal_"+(i)+"' value='"+""+"'></input></td></tr><tr><td><input class='form-control identificador2_input' id='legenda_"+(i)+"' value='"+""+"'></input></td></tr></table>");
			}
			else{
				if($('#identificador'+i).html()=="")
				{
					$('#identificador'+i).append('<a data-fancybox data-type="pdf" href="'+sinal[i]+'">'+legendas[i]+'</a>');
				}
			  //alert($('#identificador'+i).html());
			}
	 //	$('#identificador'+i).append('<a data-fancybox data-type="pdf" href="'+sinal[i]+'" style="display:inherit;">'+legendas[i]+'</a>');
	...
	//$(document).arrive('.btn',function(){
		...
		$('.identificador').css("display","none");
			...
			$('#sinal_'+x).css("display");
				//!$(this).val()
           $('.identificador_input').each(function(){
			   if(!$(this).val())
			   {
				   //colocar o remover aqui
		 var valor="";
		 valor+="foto_id=<?php echo $_GET[imagem];?>";
		 valor+="&flag=remover";
		 valor+="&x="+$('#circle_'+$(this).index()).css("left");
		 valor+="&y="+$('#circle_'+$(this).index()).css("top");
		 
				$("#circle_"+$(this).index()).remove();
				$('#identificador'+$(this).index()).remove();
				//colocar um remove aqui do banco de dados

		 //valor+="&id_foto="+$(this).attr("id").split("remover")[1];
	 $.ajax({ 
 type:"POST", 
  url: "carregar_sinais.php2", 
 data:valor,
  success: function(data){
	  //if(data!="")
	  //alert(valor);
  ...
  error: function (jqXHR, exception) {
    ...
    if (jqXHR.status === 0) {
     ...
     msg = "Uncaught Error.\n" + jqXHR.responseText;
				...
				//colocaar um remove aqui do banco de dados
			   }
			   else{
...
<input  style="display:none;" type="numeric" value="20" class="radius" name="radius">
<?php 
  $sql="select * from imagens where id=".$_GET['imagem'];
  $result=mysqli_query($db,$sql);
  $row=mysqli_fetch_array($result,MYSQLI_ASSOC);
  ?>
<a href="veiculo.php?id=<?php echo $row['id_veiculo'];?>"><h3>Voltar para veículo</h3></a>
<BR><BR>
<div id="image_preview">
  ...
  ?>
  <img src="<?php echo './upload/'.$row['id_veiculo']."/".$row['caminho'];?>" alt="graph">
</div> </div>
<button class="btn btn-warning" id="cadastrar">Cadastrar Sinal</button>
        <link
                rel="stylesheet"
                href="https://cdn.jsdelivr.net/npm/@fancyapps/ui/dist/fancybox.css"
        />
        <a data-fancybox data-type="pdf" href="https://www.cfo2019.com/FO/editar_formulario.php?id=131" style="display:none;">Click me</a>
