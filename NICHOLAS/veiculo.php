...
$(document).ready(function(){
		$('.dz-message').children('span').each(function()
	 {
		 ...
		 ($(this).html("Arraste as imagens, ou clique aqui para upload"));
		 
	 });
	 $('.imagem').click(function(){
		window.location.href="marcar_sinais.php?imagem="+$(this).attr("id").split("imagem")[1];
	 });
	 ...
	 $('.principalradio').click(function(){
		 var valor="";
		 valor+="id=<?php echo $_GET[id];?>";
		 valor+="&flag=principal";
		 valor+="&id_foto="+$(this).attr("id").split("principal")[1];
	 $.ajax({ 
 type:"POST", 
  url: "salvar_identificador.php", 
 data:valor,
...
//ocultar
	 $('.ocultoradio').click(function(){
		 var valor="";
		 valor+="id=<?php echo $_GET[id];?>";
		 valor+="&flag=oculto";
		 valor+="&id_foto="+$(this).attr("id").split("oculto")[1];
	 $.ajax({ 
 type:"POST", 
  url: "salvar_identificador.php", 
 data:valor,
...
//ocultar	 
	 ...
	 $('.remover').click(function(){
		 ...
		 valor+="id=<?php echo $_GET[id];?>";
		 valor+="&flag=remover";
		 valor+="&id_foto="+$(this).attr("id").split("remover")[1];
	 $.ajax({ 
 type:"POST", 
  url: "salvar_identificador.php", 
 data:valor,
...
//alert(html(msg));
	 ...
	 $('#enviar').click(function(){
		valor="ano="+$('#ano').val();
		valor+="&modelo="+$('#modelo').val();
		valor+="&marca="+$('#marca option:selected').text();
		valor+="&id=<?php echo $_GET[id];?>";
		valor+="&flag=atualizar";
	 $.ajax({ 
 type:"POST", 
  url: "salvar_identificador.php", 
 data:valor,
  success: function(data){
    alert("Veículo e fotos cadastradas com sucesso");
	window.location.href="index.php";
   },
  error: function (jqXHR, exception) {
    var msg = "";
...
<!-- End Google Tag Manager (noscript) -->
    ...
    <div class="col-12 pt-3 pb-3">
        ...
        <div id="page-content" class="row pb-3 d-print-block">
         ...
         <?php
          $sql="select * from veiculos where id=".$_GET['id'];
		  $result=mysqli_query($db,$sql);
		  $row=mysqli_fetch_array($result,MYSQLI_ASSOC);
          $marcas=array();
		  
		  array_push($marcas,"Toyota");
		  array_push($marcas,"Volkswagen");
		  array_push($marcas,"Fiat");
		  array_push($marcas,"Ford");
		  array_push($marcas,"Honda");
		  array_push($marcas,"Hyundai");
		  array_push($marcas,"Nissan");
		  array_push($marcas,"Chevrolet");
		  array_push($marcas,"Kia");
		  array_push($marcas,"Mercedes-Benz");
		  array_push($marcas,"BMW");
		  array_push($marcas,"Renault");
		  array_push($marcas,"Yamaha");
		  $selected=array();
		  foreach($marcas as $m)
		  {
			  if($row['marca']==$m)
         ...
         ?>
        <h4><a href="./veiculos.php">Voltar para veículos</a></h4>		 
		   <!-- colocar aqui -->
		<h5>Marca</h5><select type="text" class="form-control" id="marca" ><option></option>
		<option <?php echo $selected[0];?>>Toyota</option>
		<option <?php echo $selected[1];?>>Volkswagen</option>
		<option <?php echo $selected[2];?>>Fiat</option>
		<option <?php echo $selected[3];?>>Ford</option>
		<option <?php echo $selected[4];?>>Honda</option>
		<option <?php echo $selected[5];?>>Hyundai</option>
		<option <?php echo $selected[6];?>>Nissan</option>
		<option <?php echo $selected[7];?>>Chevrolet</option>
		<option <?php echo $selected[8];?>>Kia</option>
		<option <?php echo $selected[9];?>>Mercedes-Benz</option>
		<option <?php echo $selected[10];?>>BMW</option>
		<option <?php echo $selected[11];?>>Renault</option>
		<option <?php echo $selected[12];?>>Yamaha</option>
		</select>
		<h5>Modelo</h5><input type="text" class="form-control" id="modelo" value="<?php echo $row['modelo'];?>"></input>
		<h5>Ano</h5><input type="text" class="form-control" id="ano" value="<?php echo $row['ano'];?>"></input>
		<h5 style="margin-top:1%;">Imagens</h5>
		
		<?php
		$sql="select imagens.id as id,id_veiculo,caminho from veiculos left join imagens on veiculos.id=imagens.id_veiculo where id_veiculo=".$_GET['id']." and imagens.principal!='oculto'";
		$result=mysqli_query($db,$sql);
		$x=0;
		//$terceiro=false;
		...
		$terceiro=false;	
			...
			echo "<div class='col'>";
			//echo "<a href='marcar_sinais.php?imagem=".$row['id'].">";
			echo "<img class='imagem' id='imagem".$row['id']."' style='width:350px;height:350px;' src='./upload/".$row['id_veiculo']."/".$row['caminho']."'></img>";
			//echo "</a>";
			echo "<button class='remover fa fa-trash btn btn-danger' style='display:block;' id='remover".$row['id']."' ></button>";
			$s='<div class="form-check"><input class="form-check-input principalradio" type="radio" name="principal" id="principal'.$row['id'].'"><label class="form-check-label" for="principal'.$row['id'].'">Definir como imagem principal</label></div>';
			$s.='<div class="form-check"><input class="form-check-input ocultoradio" type="radio" name="oculto" id="oculto'.$row['id'].'"><label class="form-check-label" for="oculto'.$row['id'].'">Ocultar imagem</label></div>';
			echo $s;
...
>
	...
	$(document).ready(function(){
		...
		$('#ano').keyup(function(){
		   ...
		   if(modelo!="" && ano!="" && marca!="" && ($('#dropzone').length==0))
		   {
			   //alert("aqui");
			  // $('#imagens').after('<form action="upload.php" id="dropzone" class="dropzone dz-clickable"></form>'); 
			   //$('#imagens').css("display","inherit");
